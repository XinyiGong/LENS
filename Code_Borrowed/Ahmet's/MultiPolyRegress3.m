function reg = MultiPolyRegress3(Data,R,PW,varargin)
...
%   Copyright (c) 2015, Ahmet Cecen  -  All rights reserved.
    ...
    if strcmp(FigureSwitch,'figureon')==1
        ...
        subplot(1,2,1);
        plot(yhat,R,'ko','MarkerFaceColor','cyan');
        set(gca,'FontSize',12);
        xlabel('Data Prediction','FontSize',14);
        ylabel('Target','FontSize',14);
        title('All Data','FontSize',14);
        line([min([yhat,R]),max([yhat,R])],[min([yhat,R]),max([yhat,R])],'LineWidth',2,'color','black');
        axis tight
        axis square
        grid on
        
        subplot(1,2,2);
        plot(ycvhat,R,'ko','MarkerFaceColor','cyan');
        set(gca,'FontSize',12);
        xlabel('Data Prediction','FontSize',14);
        ylabel('Target','FontSize',14);
        title('LOOCV','FontSize',14);
        line([min([ycvhat,R]),max([ycvhat,R])],[min([ycvhat,R]),max([ycvhat,R])],'LineWidth',2,'color','black');
        axis tight
        axis square
        grid on
